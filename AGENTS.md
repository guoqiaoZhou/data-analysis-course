...
- v0.4 ✅ 端到端实战项目（3个项目，notebook修复与验证完成）
- v0.5 ✅ 反常识内容（100个坑 + 10个知识点反常识章节 + lessons-learned.md）
- v0.6-v1.0 📋 软技能深化、诊断测试、内容补全（规划中）
...
└── curriculum/                        # 课程核心内容
    ...
    │
    ├── appendix/                      # 附录（v0.5已填充）
    │   ├── 01-math-refresher/         # 数学基础速查（45+公式）
    │   ├── 02-code-templates/         # 代码模板库（18个模板）
    │   ├── 03-datasets/               # 案例数据集（3个项目数据字典）
    │   ├── 04-resources/              # 推荐资源清单（论文/教材/工具）
    │   └── 05-anti-patterns/          # 反常识内容（100个坑 + lessons-learned.md）
    │
    ├── review/                        # 审校报告
...
### 每次迭代后的检查清单
完成任何版本迭代后，必须检查以下文件是否需要同步更新：

1. **README.md**：课程概览、版本状态、项目表格是否反映最新状态
2. **curriculum/roadmap/versions.md**：当前版本详情、完成状态、日期
3. **curriculum/roadmap/future-features.md**：P0/P1功能状态是否准确
4. **curriculum/appendix/**：
   - `01-math-refresher/`：新增知识点是否包含新公式
   - `02-code-templates/`：新增知识点是否包含可提取的代码模板
   - `03-datasets/`：新增项目是否补充数据字典
   - `04-resources/`：新增知识点是否补充参考资源
5. **data-analysis-advanced.md**：方法速查表、规范手册是否与课程同步
6. **AGENTS.md**：当前版本号、完成状态是否准确

> **规范**：版本号变更（如v0.5→v0.6）必须同时更新上述所有文件中的版本引用。
