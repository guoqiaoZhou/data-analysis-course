...
for var, name in [(age, '年龄'), (income, '收入')]:
    mean_treat = np.mean(var[D_complete == 1])
    mean_control = np.mean(var[D_complete == 0])
    pooled_std = np.sqrt((np.var(var[D_complete == 1]) + np.var(var[D_complete == 0])) / 2)
...
ax.axvline(np.mean(ate_estimates), color='blue', linestyle='--', label=f'估计均值={np.mean(ate_estimates):.2f}')
